...
from portfolio_parser import parse_portfolio_text, normalize_skills
...
def _save_parsed_portfolio(parsed, clear_existing=False):
    ...
    # 기술
    existing_skills = {s["skill"].casefold() for s in get_skills()}
    for skill in normalize_skills(parsed["skills"]):
        if skill.casefold() not in existing_skills:
            add_skill(skill)
...
def _cleanup_existing_skills():
    skills = get_skills()
    normalized = normalize_skills([skill["skill"] for skill in skills])
    ...
    removed_count = max(0, len(skills) - len(normalized))
    changed_count = sum(1 for before, after in zip([s["skill"] for s in skills], normalized) if before != after)
    return len(normalized), removed_count, changed_count
...
# ===========================
    ...
    with skill_col1:
        st.caption("가져오기 시 문장형 설명은 자동 제외합니다.")
    with skill_col2:
        if skills and st.button("기술 자동 정리", use_container_width=True):
            kept_count, removed_count, changed_count = _cleanup_existing_skills()
            st.session_state.skill_cleanup_message = f"기술 {kept_count}개를 남기고 {removed_count}개를 제거했습니다."
            if changed_count:
                st.session_state.skill_cleanup_message += " 형식도 함께 정리했습니다."
            st.rerun()

    if st.session_state.get("skill_cleanup_message"):
        st.success(st.session_state.pop("skill_cleanup_message"))
    ...
    with st.form("add_skill"):
        ...
        if st.form_submit_button("추가"):
            cleaned_skills = normalize_skills([new_skill])
            if cleaned_skills:
                add_skill(cleaned_skills[0])
                st.success(f"'{cleaned_skills[0]}' 추가됨")
                st.rerun()
            elif new_skill.strip():
                st.error("기술명만 간단히 입력해주세요. 예: Python, React, AWS")
